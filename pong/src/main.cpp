...
void restartGame(player &player_01, player &player_02, ball &b) { //function to reset game variables
	...
	player_01.pos.y = SCREEN_HEIGHT - player_01.pos.h;
	player_02.pos.y = SCREEN_HEIGHT - player_02.pos.h;
...
int main(int argc, char **argv) {
	...
	b.x = SCREEN_WIDTH; //ball x position
	b.y = SCREEN_HEIGHT; //ball y position
	...
	player_01.speed = 20; // player 1 speed
    ...
    //Setup window
    SDL_Window* window = SDL_CreateWindow("Main-Window", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, SDL_WINDOW_FULLSCREEN_DESKTOP);
	...
	SDL_Surface* paddleImage2 = IMG_Load("romfs:/res/image/paddle2.png");
	...
	SDL_Texture* paddleTexture2 = SDL_CreateTextureFromSurface(renderer, paddleImage2);
	SDL_Texture* ballTexture = SDL_CreateTextureFromSurface(renderer, ballImage);
	
	//Add background Images to Textures
//	SDL_Surface* backImage = IMG_Load("romfs:/res/image/background2.png");
...
//loads to slow
	...
	Mix_Music *audio = Mix_LoadMUS("romfs:/res/audio/test.mp3");
		...
		if (state == GAME_SINGLE_PLAYER) { //If Single player mode
			...
			if (!paused) {
				...
				if (b.x < 0) {
					...
					b.y = player_01.pos.y + player_01.pos.h/1;
				...
				if (b.x + BALL_WIDTH >= SCREEN_WIDTH) {
					...
					b.y = player_02.pos.y + player_02.pos.h /1;
